...
// clang-format off
DEFINE_FSTR_ARRAY_LOCAL(glcdfont, uint8_t,
...
LcdGlyph::LcdGlyph(size_t bmOffset, const Options& options) : GlyphObject(metrics, options)
{
	Size sz{rawSize};
	uint8_t bits[sz.w];
	glcdfont.read(bmOffset, bits, sz.w);
	for(unsigned row = 0; row < sz.h; ++row) {
		for(unsigned col = 0; col < sz.w; ++col) {
			rowBits[row][col] = bits[col] & (0x01 << row);
		}
	}
}

void LcdGlyph::readAlpha(void* buffer, Point origin, size_t stride) const
{
	auto bufptr = static_cast<uint8_t*>(buffer) + origin.x + origin.y * stride;
	for(unsigned y = 0; y < rawSize.h; ++y) {
		auto bits = getBits(y);
		for(unsigned x = 0; x < rawSize.w; ++x) {
			if(bits[x]) {
				bufptr[x] = 0xff;
			}
		}
